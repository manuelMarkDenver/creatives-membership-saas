...
export const locationsApi = {
  ...
  getAll: async (params?: LocationQueryParams) => {
    const response = await apiClient.get('/api/v1/gym/locations', { params })
  ...
  getByTenant: async (tenantId: string, params?: Omit<LocationQueryParams, 'tenantId'>) => {
    ...
    const response = await apiClient.get('/api/v1/gym/locations', { 
      params: { ...params },
  ...
  getById: async (id: string): Promise<Location> => {
    const response = await apiClient.get(`/api/v1/gym/locations/${id}`)
    return response.data
  },

  // Create location
  create: async (data: CreateLocationDto): Promise<Location> => {
    const response = await apiClient.post('/api/v1/gym/locations', data)
    return response.data
  },

  // Update location
  update: async (id: string, data: UpdateLocationDto): Promise<Location> => {
    const response = await apiClient.put(`/api/v1/gym/locations/${id}`, data)
    return response.data
  },

  // Delete location
  delete: async (id: string): Promise<void> => {
    await apiClient.delete(`/api/v1/gym/locations/${id}`)
  },

  // Get location stats/analytics
  getStats: async (id: string) => {
    const response = await apiClient.get(`/api/v1/gym/locations/${id}/stats`)
    return response.data
  },

  // Assign staff to location
  assignStaff: async (locationId: string, data: { userId: string; accessLevel: string }) => {
    const response = await apiClient.post(`/api/v1/gym/locations/${locationId}/staff`, data)
    return response.data
  },

  // Update staff location access
  updateStaffAccess: async (locationId: string, userId: string, data: { accessLevel: string }) => {
    const response = await apiClient.put(`/api/v1/gym/locations/${locationId}/staff/${userId}`, data)
    return response.data
  },
}
