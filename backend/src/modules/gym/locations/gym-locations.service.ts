...
export class GymLocationsService {
  ...
  async createGymLocation(createLocationDto: CreateBranchDto) {
    try {
      if (!createLocationDto.tenantId) {
        throw new BadRequestException('Tenant ID is required to create a gym location');
      }
      ...
      throw new ConflictException('Gym location creation failed due to conflict');
    }
  }

  async updateGymLocation(locationId: string, updateLocationDto: UpdateBranchDto) {
    const location = await this.prisma.branch.findUnique({ where: { id: locationId } });
    if (!location) {
  ...
  async findAllGymLocations(tenantId: string) {
    return this.prisma.branch.findMany({
      ...
      return locations.map(location => {
        const members = location.userBranches.filter(ub => ub.user.role === 'GYM_MEMBER');
        const activeMembers = members.filter(ub => ub.user.isActive).length;
        const inactiveMembers = members.filter(ub => !ub.user.isActive).length;
  ...
  async assignStaffToLocation(assignUserDto: AssignUserToBranchDto, locationId: string) {
    const location = await this.prisma.branch.findUnique({ where: { id: locationId } });
    if (!location) {
      throw new NotFoundException('Gym location not found');
    }
  ...
  async updateStaffLocationAccess(updateAccessDto: UpdateUserBranchAccessDto, userId: string, locationId: string) {
