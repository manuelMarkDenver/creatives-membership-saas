...
export class BranchesService {
  ...
  async findAllBranches(tenantId: string) {
    return this.prisma.branch.findMany({
      ...
      return branches.map(branch => {
        const members = branch.userBranches.filter(ub => ub.user.role === 'GYM_MEMBER');
  ...
  async assignUserToBranch(assignUserToBranchDto: AssignUserToBranchDto, branchId: string) {
    const branch = await this.prisma.branch.findUnique({ where: { id: branchId } });
    if (!branch) {
      throw new NotFoundException('Branch not found');
    }
  ...
  async updateUserBranchAccess(updateUserBranchAccessDto: UpdateUserBranchAccessDto, userId: string, branchId: string) {
